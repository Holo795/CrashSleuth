...
- **« Corrigez vos datapacks », répondu à des gens qui n'en ont aucun.** Trois signalements recevaient ce conseil du jeu, répété tel quel :
  - `Unbound values in registry… [terralith:…]` : tous les identifiants portent le nom d'un mod, et c'est un *autre* mod (celui qui ajoute la compatibilité) qui les réclame. Rejoué au labo sur Fabric 26.2 avec les deux jars du signalement, mêmes identifiants ;
...
- **La console du lanceur ou du panel est lue** (`console.log` à côté du journal du jeu) : un pilote graphique qui refuse la fenêtre, une JVM qui ne démarre pas, un loader qui meurt avant l'ouverture du journal — rien de tout ça n'atteint `latest.log`. Vérifié au labo en faisant croire au pilote qu'il n'a qu'OpenGL 2.1 : le jeu n'écrit que « GLFW error 65543 » dans la console, et CrashSleuth explique qu'il faut OpenGL 3.2 depuis la 1.17 (et que la phrase du jeu sur les pilotes est un texte générique, pas un diagnostic).
- **Les couples connus marchent aussi sur un journal seul** : les loaders impriment la liste des mods chargés, ce qui suffit à nommer une paire incompatible même sans les fichiers.
- **Modèles manquants d'un pack** signalés en confiance basse, avec la précision que ça ne fait pas planter le jeu (c'est la confusion la plus fréquente dans les rapports), et **rien n'est signalé** pour les connexions qui n'ont jamais dit bonjour (scanners de ports).
- **Six signalements non reproductibles, écartés** plutôt que mis en scène (crash Sodium `@Overwrite`, configuration de mod vidée, cycle de chargement de plugins, Jade + Not Enough Crashes, registre gelé d'Immersive Melodies, entités au même UUID).
