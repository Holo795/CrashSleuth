...
- “OpenGL is not supported” on a card made in the last decade is not about the card: on Linux the game often runs through XWayland, and the players who hit it start with SDL_VIDEO_DRIVER=wayland;
...
Tested in the lab, on every line of versions from **1.19 to the latest** ([exact table](https://holo795.github.io/CrashSleuth/Supported-versions.html)): vanilla, Paper, Spigot, Purpur, Folia, Fabric, Quilt, NeoForge and Forge servers; the Mohist, Youer and Arclight hybrids, which run mods and plugins at once; Velocity, BungeeCord and Waterfall proxies; vanilla, Fabric, NeoForge and Forge clients.
